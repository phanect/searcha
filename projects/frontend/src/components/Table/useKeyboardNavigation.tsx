 ...
 * @param root0
 * @param root0.gridRef
 * @param root0.tableRows
 * @param root0.leafColumns
 */
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === "Enter") {
        ...
        (target.querySelector("[tabindex]"))?.focus();
        return;
      }
      ...
      switch (e.key) {
        ...
        case "ArrowDown":
          if (e.ctrlKey || e.metaKey) {
            newRowIndex = tableRows.length - 1;
      ...
      // Focus the cell
      if (newCellEl) {
        setTimeout(() => (newCellEl as HTMLDivElement).focus());
      }
    },
